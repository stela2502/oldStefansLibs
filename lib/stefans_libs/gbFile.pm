...
sub new {
	...
	my ( $self, @features, %feature_locations );
	$self = {
		debug             => $debug,
		name              => undef,
		path              => undef,
...
sub SelectNotMatchingFeatures {
	...
	foreach $feature (@$features) {
		$temp = 0;
		foreach $searchString (@$featureNames) {
...
sub Features {
	...
	#  print "gbFeature Feature = $feature\n";
...
sub getAsGB {
	my ( $self, $notInclude ) = @_;

	my $string = '';
	$notInclude = 'header features sequence' unless ( defined $notInclude );

	if ( $notInclude =~ m/header/ ) {
		$string .= $self->{header}->getAsGB();
	}

	if ( $notInclude =~ m/features/ ) {
...
sub Length {
	...
	my ( $self, $length ) = @_;
	$self->{seq_length} = $length if ( defined $length );
