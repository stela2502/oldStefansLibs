...
# I now need an hash containing the keys:
#im, x_min, x_max, y_min, y_max, color, fillColor, x_title, y_title, portrait, fixed_axis, fixed_axis_is
...
sub AddData {
	...
	#Carp::confess ( print root::get_hashEntries_as_string ( $self->{y_hist} , 3 , "densityMap.pm - the y hist" ));
	...
	for ( my $i = 0 ; $i < @$x_array ; $i++ ) {
		...
		unless ( defined @$matrix[$x] ) {
			warn
"we have a serious problem here -> the x value $x (@$x_array[$i]) was not initialized in the matrix!\n",
"Therefore I have dropped the x/y data value @$x_array[$i]/@$y_array[$i] information!\n";
			next;
		}
		@$matrix[$x]->[$y]++;
