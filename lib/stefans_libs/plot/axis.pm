...
sub new {
	...
	if ( $resolution eq "small" ) {
		$self->{tics}       = 8;
		$self->{tic_length} = 17;
		$self->{font}       = Font->new($resolution);
	}
	if ( $resolution eq "tiny" ) {
		$self->{tics}       = 6;
		$self->{tic_length} = 7;
		$self->{font}       = Font->new($resolution);
	}
...
sub resolveValue {
	...
	#    print "axis: resolveValue $value\n";
	#$self->defineAxis();
	Carp::confess("Hey why do you want to plot a 'No Value'?")
	  if ( $value eq "No Values" );
	if ( $value > $self->max_value() ) {
		Carp::confess(
			    "we have a deep recursion here - please fix that in the code\n"
			  . "You want to get a value $value that is above our max value "
			  . $self->max_value()
			  . "\n for the second time - that should NOT be possible!\n" )
		  if ( $self->{'recursion'} == 1 );
	...
	if ( $value < $self->min_value() ) {
		Carp::confess(
			    "we have a deep recursion here - please fix that in the code\n"
			  . "You want to get a value $value that is below our min value "
			  . $self->min_value()
			  . "\n for the second time - that should NOT be possible!\n" )
		  if ( $self->{'recursion'} == 1 );
		print "we need to set your call to " . $self->min_value() . "\n";
		return $self->resolveValue( $self->min_value() );
		$self->{'recursion'} = 1;
	}
	$self->{'recursion'} = 0;
