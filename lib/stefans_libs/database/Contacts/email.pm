package stefans_libs_database_Contacts_email;
...
sub new {
    ...
    bless $self, $class if ( $class eq "stefans_libs_database_Contacts_email" );
...
sub  init_tableStructure {
     ...
     push ( @{$hash->{'UNIQUES'}}, [ 'email' ]);

     $self->{'table_definition'} = $hash;
     $self->{'UNIQUE_KEY'} = [ 'email' ];
