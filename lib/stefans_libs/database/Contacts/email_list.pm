package stefans_libs_database_Contacts_email_list;
...
use stefans_libs::database::Contacts::email;
...
stefans_libs_database_Contacts_email_list

=head1 DESCRIPTION

A list table that links to the table stefans_libs_database_Contacts_email.

=head2 depends on

stefans_libs::database::Contacts::email
...
new returns a new object reference of the class stefans_libs_database_Contacts_email_list.
...
sub new {
	...
	$self = {
		debug           => $debug,
		dbh             => $dbh,
		'my_table_name' => "email_list"
	};

	bless $self, $class
	  if ( $class eq "stefans_libs_database_Contacts_email_list" );

	$self->init_tableStructure();
	$self->{'data_handler'}->{'otherTable'} =
	  stefans_libs_database_Contacts_email->new( $self->{'dbh'},
		$self->{'debug'} );
