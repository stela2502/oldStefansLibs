package stefans_libs_database_Contacts_telephone;
...
sub new {
    ...
    bless $self, $class if ( $class eq "stefans_libs_database_Contacts_telephone" );
...
sub  init_tableStructure {
     ...
     push ( @{$hash->{'UNIQUES'}}, [ 'type', 'number' ]);

     $self->{'table_definition'} = $hash;
     $self->{'UNIQUE_KEY'} = [ 'type', 'number' ];
