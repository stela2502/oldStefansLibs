package stefans_libs_database_sequenome_data_sequenome_quality;
...
sub new {
    ...
    bless $self, $class if ( $class eq "stefans_libs_database_sequenome_data_sequenome_quality" );
...
sub  init_tableStructure {
     ...
     $hash->{'UNIQUES'}   = [];
     $hash->{'variables'} = [];
     $hash->{'table_name'} = "sequenome_quality";
     push ( @{$hash->{'variables'}},  {
               'name'         => 'quality_tag',
               'type'         => 'VARCHAR (200)',
               'NULL'         => '0',
     ...
     push ( @{$hash->{'UNIQUES'}}, [ 'quality_tag' ]);

     $self->{'table_definition'} = $hash;
     $self->{'UNIQUE_KEY'} = [ 'quality_tag' ];
