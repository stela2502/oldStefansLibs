package stefans_libs_database_sequenome_data_sequenome_assays;
...
sub new {
    ...
    bless $self, $class if ( $class eq "stefans_libs_database_sequenome_data_sequenome_assays" );
...
sub  init_tableStructure {
     ...
     $hash->{'UNIQUES'}   = [];
     $hash->{'variables'} = [];
     $hash->{'table_name'} = "sequenome_assays";
     push ( @{$hash->{'variables'}},  {
               'name'         => 'assay_name',
               'type'         => 'VARCHAR (200)',
               'NULL'         => '0',
     ...
     push ( @{$hash->{'UNIQUES'}}, [ 'assay_name' ]);

     $self->{'table_definition'} = $hash;
     $self->{'UNIQUE_KEY'} = [ 'assay_name' ];
