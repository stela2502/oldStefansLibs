package stefans_libs_LabBook_file_list;
...
use stefans_libs::database::LabBook::file_list::files_table;
...
sub new {
	...
	$self = {
		debug           => $debug,
		dbh             => $dbh,
		'my_table_name' => "LabBook_file_list"
	};

	bless $self, $class if ( $class eq "stefans_libs_LabBook_file_list" );

	$self->init_tableStructure();
	$self->{'data_handler'}->{'otherTable'} =
	  stefans_libs_LabBook_files_table->new( $self->{'dbh'}, $self->{'debug'} );
...
sub get_file_data_for_id {
	my ( $self, $id ) = @_;
	Carp::confess("Sorry, but I would need an id!\n") unless ( defined $id );
	return $self->{'data_handler'}->{'otherTable'}->get_file_data_for_id(
