...
sub new {
	...
	$self = {
		...
		debug => $debug,
		'DEBUG' =>
		  1,    ## set this variable manually to debug the web page parsing!
...
"The score was calculated by matching these strings to the description of the gene: "
		...
		my ($filename);
#		if ( defined $hash->{'data_file'} ) {
#			## hui - now we should plot some infos here....
#			## But the last update has unfortunately killed the abillity to use this function here!
#			## I therefore need to plot all genes using the functionallity I use in plot_best_batch_result.
#			## that is tedious, but it will get me to the right result!
#			
#			my $text = $gene_section->AddText(
#"The expression for this gene is depicted in figure \\ref{gene::figure::$gene"."::0} on page \\pageref{gene::figure::$gene"."::0}."
#				);
#				my $figure = $text->Add_Figure();
#				$figure->AddPicture(
#					{
#						'placement' => 'htbp',
#						'files'     => [
...
#						],
#						'caption' =>
#"The expression of the gene $gene in the here analyzed dataset. Depicted are the Probe Set ID(s) "
#						  . join( "; ",
#							keys %{ $hash->{'probesets'}->{$gene} } )
#						  . ".",
#						'subfigure_captions' =>
#						  [ keys %{ $hash->{'probesets'}->{$gene} } ],
#						'width' => '0.5',
#						'label' => "gene::figure::$gene"
#					}
#				);
#			}
...
#print "\n\n".ref($self)."::_get_summary_datastructure we get the data for the genes ".join(" ", @{ $hash->{genes} })."\n"
#."and desease $hash->{'desease'}\n";
	foreach my $gene ( @{ $hash->{genes} } ) {

		#print "we try to get the data for gene $gene\n";
		$result->{$gene} = {};
		(
			$result->{$gene}->{'summary'},
			$result->{$gene}->{'score'},
...
sub digest_description {
	...
	#print "we will digest the description $description\n";
...
sub get_href_for_gene {
	my ( $self, $gene ) = @_;
	return "http://www.genecards.org/cgi-bin/carddisp.pl?gene=$gene";
}
...
sub _get_gene_description_from_genecards {
	...
	#	if ( -f "gene_description_error.log" ) {
	#		open( ERR, ">>gene_description_error.log" )or die
	#		  "could not create the error log 'gene_description_error.log'\n$!\n";
	#	}
	#	else {
	#		open( ERR, ">gene_description_error.log" )
	#		  or die
	#		  "could not create the error log 'gene_description_error.log'\n$!\n";
	#	}
...
sub __score_line {
	...
	my @score = ();
	my $score = 0;
	return $score unless ( defined $line );
	foreach (
	...
	foreach ( 'http://', 'href', 'onmouseover', 'border', 'myArray', 'tmp' ) {
		foreach ( $line =~ m/$_/g ) {
			push( @score, $_ );
			$score--;
		}
	...
	return $score, @score if ( $self->{'DEBUG'} );
	return $score;
}
...
#Carp::confess( ref($self)."::__get_hash_4_genecards_url we try to get '$url'\n");
	my @error = eval {
		$self->{'mech'}->get($url);
	...
	foreach ( split( "\n", $data ) ) {
		$print = 1 if ( $_ =~ m/RefSeq/ );
		$print = 1 if ( $_ =~ m/UniProtKB/ );
		$print = 1;
		if ($print) {
			## now I can create a score!
			@temp = $self->__score_line($_);
			$data_table->AddDataset(
				{
					'score'   => $temp[0],
					'line nr' => $i,
					'str'     => $self->__drop_HTML($_)
				}
			);
			$print = 0;
		}
