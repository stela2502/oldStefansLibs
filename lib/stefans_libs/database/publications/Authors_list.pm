...
use stefans_libs::database::publications::Authors;
...
sub new {
	...
	$self = {
		debug => $debug,
		dbh   => $dbh,
		'my_table_name' => "Authors_list"
	};

	bless $self, $class if ( $class eq "Authors_list" );

	$self->init_tableStructure();
	$self->{'data_handler'}->{'otherTable'} =
	  Authors->new( $self->{'dbh'}, $self->{'debug'} );
