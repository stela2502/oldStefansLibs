package stefans_libs_database_school_math_test;
...
sub new {
    ...
    bless $self, $class if ( $class eq "stefans_libs_database_school_math_test" );
...
sub  init_tableStructure {
     ...
     push ( @{$hash->{'UNIQUES'}}, [ 'send_time' ]);

     $self->{'table_definition'} = $hash;
     $self->{'UNIQUE_KEY'} = [ 'send_time' ];
