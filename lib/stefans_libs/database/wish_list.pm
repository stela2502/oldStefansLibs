...
sub new {
    ...
    bless $self, $class if ( $class eq "wish_list" );
...
sub  init_tableStructure {
     ...
     push ( @{$hash->{'UNIQUES'}}, [ 'scientist_id', 'md5_sum' ]);

     $self->{'table_definition'} = $hash;
     $self->{'UNIQUE_KEY'} = [ 'scientist_id', 'md5_sum' ];
	
     $self->{'table_definition'} = $hash;

     $self->{'Group_to_MD5_hash'} = ['whish' ]; # define which values should be grouped to get the 'md5_sum' entry
