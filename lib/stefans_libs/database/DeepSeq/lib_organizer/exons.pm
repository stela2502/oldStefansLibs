package stefans_libs_database_DeepSeq_lib_organizer_exons;
...
sub new {
    ...
    bless $self, $class if ( $class eq "stefans_libs_database_DeepSeq_lib_organizer_exons" );
...
sub  init_tableStructure {
     ...
     push ( @{$hash->{'variables'}},  {
               ...
               'type'         => 'INTEGER UNSIGNED',
               'NULL'         => '0',
               'description'  => 'the end in bp',
          }
     );

     $self->{'table_definition'} = $hash;
     $self->{'UNIQUE_KEY'} = [ 'chr','start','end' ];
