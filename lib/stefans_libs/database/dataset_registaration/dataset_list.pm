...
use stefans_libs::database::dataset_registration;
...
sub new {
	...
	$self = {
		debug => $debug,
		dbh   => $dbh,
		'my_table_name' => "dataset_list"
	};

	bless $self, $class if ( $class eq "dataset_list" );

	$self->init_tableStructure();
	$self->{'data_handler'}->{'otherTable'} =
	  dataset_registration->new( undef, $self->{'debug'} );
