package stefans_libs_database_Company_Invoices;
...
use stefans_libs::database::Company::Contracts;
...
sub new {
    ...
    bless $self, $class if ( $class eq "stefans_libs_database_Company_Invoices" );
...
sub  init_tableStructure {
     ...
     $hash->{'UNIQUES'}   = [];
     $hash->{'variables'} = [];
     $hash->{'table_name'} = "Invoices";
     push ( @{$hash->{'variables'}},  {
               'name'         => 'contract_id',
               'type'         => 'INTEGER UNSIGNED',
               'NULL'         => '0',
               'description'  => '',
               'data_handler' => 'stefans_libs_database_Company_Contracts',
          }
     );
     push ( @{$hash->{'variables'}},  {
               'name'         => 'send_date',
     ...
     push ( @{$hash->{'UNIQUES'}}, [ 'contract_id', 'send_date' ]);

     $self->{'table_definition'} = $hash;
     $self->{'UNIQUE_KEY'} = [ 'contract_id', 'send_date' ];
     ...
     $self->{'data_handler'}->{'stefans_libs_database_Company_Contracts'} = stefans_libs_database_Company_Contracts->new($self->{'dbh'}, $self->{'debug'});
