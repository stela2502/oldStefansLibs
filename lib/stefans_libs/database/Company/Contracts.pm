package stefans_libs_database_Company_Contracts;
...
use stefans_libs::database::Company::Offers;
...
sub new {
    ...
    bless $self, $class if ( $class eq "stefans_libs_database_Company_Contracts" );
...
sub  init_tableStructure {
     ...
     $hash->{'UNIQUES'}   = [];
     $hash->{'variables'} = [];
     $hash->{'table_name'} = "Contracts";
     push ( @{$hash->{'variables'}},  {
               'name'         => 'offers_id',
               'type'         => 'INTEGER UNSIGNED',
               'NULL'         => '0',
               'description'  => '',
               'data_handler' => 'stefans_libs_database_Company_Offers',
          }
     );
     push ( @{$hash->{'variables'}},  {
               'name'         => 'date',
     ...
     $self->{'data_handler'}->{'stefans_libs_database_Company_Offers'} = stefans_libs_database_Company_Offers->new($self->{'dbh'}, $self->{'debug'});
