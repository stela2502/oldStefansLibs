...
sub add_LaTeX_section_for_Gene_List {
	...
	my $table = $self->__get_table_for_gene_list( $hash->{'genes'} );
...
"Sorry, but I could not find any KEGG pathway that described any of these genes:\n"
	...
	$table = $self->__resrict_results_table($table);
	...
	#Carp::confess ( "Is this summary_table what I would need??\n".$summary_table->AsString() );
	if ( scalar( @{ $summary_table->{'data'} } ) == 0 ) {
		$hash->{'LaTeX_object'}->Section('KEGG pathways')
	...
	$temp_text->Add_Table( $summary_table->get_as_table_object('plottable') );
...
sub identify_significant_KEGG_pathways {
	my ( $self, $table, $kegg_reference_geneset, $only_significants,
		$available_genes )
	  = @_;
	my ( $summary_table, $hypergeometric_max_hits, $reference_dataset,
		$gene_count, $temp );

	$summary_table = $self->__gene_hits_for_each_KEGG_pathway($table);
...
#						&hypergeom( $_[0], $_[1], $gene_count, $_[2] ) )."\n";
					return sprintf( '%.1E',
						&more_hypergeom( $_[0], $_[1], $gene_count, $_[2] ) );
				},
				'data_column'   => 'data',
				'target_column' => 'hypergeometric p value'
			}
		);
		$summary_table = $summary_table->Sort_by(
...
=head2 __get_table_for_gene_list ( [ genes ])
...
sub __resrict_results_table {
	...
	my $summary_table =
	  $self->__gene_hits_for_each_KEGG_pathway($table);
...
sub __gene_hits_for_each_KEGG_pathway {
	my ( $self, $table ) = @_;
	$table->define_subset( 'hyper_data', [ 'Gene_Symbol', 'pathway_name' ] )
	  unless ( defined $table->Header_Position('hyper_data') );
