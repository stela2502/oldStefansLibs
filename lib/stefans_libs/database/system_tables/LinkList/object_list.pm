...
use stefans_libs::database::system_tables::LinkList::www_object_table;
...
sub new {
	...
	$self = {
		debug => $debug,
		dbh   => $dbh,
		'my_table_name' => "object_list"
	};

	bless $self, $class if ( $class eq "object_list" );

	$self->init_tableStructure();
	$self->{'data_handler'}->{'otherTable'} =
	  www_object_table->new( $self->{'dbh'}, $self->{'debug'} );
