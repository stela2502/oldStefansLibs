...
sub new {
	...
	die "New gbFeature needs a tag info and a region! ( '$class', '$tag', '$region')\n" unless ( @_ == 3 );
	...
	if ( length($self->{'tag'}) > 14 ){
		$self->{'tag'} = substr( length($self->{'tag'} ),length($self->{'tag'})- 14, 14);
	}
...
sub getPromoterRegion{
	...
	if ( $self->IsComplement () =~ m/\w/ ){
		return ($self->End () - $downstream_add), ($self->End () + $upstream_add) ;
	}
	else {
...
sub getAsGB {
	...
	foreach my $string ( keys %{$self->{match}} ) {
		$line = $self->_feature2string( undef, $string,
			"                     /note=\"match mode " );
		$temp = chop $line;
...
sub AddInfo {
	...
	$self->{information}->{$tag} = [] unless ( defined $self->{information}->{$tag} );
...
sub DropInfo{
	...
	unless ( defined $info){
		delete $self->{information}->{$tag} if ( ref($self->{information}->{$tag}) eq "ARRAY");
	}
	else {
...
sub selectValue_from_tag_str{
	...
	unless ( $matchingStr =~ m/\(/ && $matchingStr =~ m/\)/ ){
		$self->{error} .= ref($self).":selectValue_from_tag_str -> the selection sting '$matchingStr' does not contain a retrieval tag ()!\n";
	}
...
sub _getFeatureStr{
	my ( $self, $tag ) = @_;
	unless ( defined $tag ) {
		$self->{error} .= ref($self).":_getFeatureStr -> we did not get a tag information - therefore we do not know which string you want!\n";
	}
	unless ( ref ($self->{information}->{$tag}) eq "ARRAY" ){
		$self->{error} .= ref($self).":_getFeatureStr -> we do not have information about the tag '$tag'!\n";
		return undef;
	}
	else {
		my $return;
