...
sub new {
	...
	my (@items);
	my $self = { items => \@items, regions => undef };
...
sub Print {
	...
	return "1..2" unless ( defined @$regions );
...
#    print "AA gbRegion ChangeRegion_Add new region info = ", $self->Print(0,$self->End()),"\n";
...
#    print "A gbRegion ChangeRegion_Add new region info = ", $self->Print(0,$self->End()),"\n";

	$self->Region( $self->Print( 0, $self->End() + $bp_to_add * 2 ) )
	  if ( $bp_to_add > 0 );
	$self->Region( $self->Print( 0, $self->End() ) ) if ( $bp_to_add < 0 );

#    print "B gbRegion ChangeRegion_Add new region info = ", $self->Print(0,$self->End()),"\n";
	return 1;
}
...
#  print "AA gbRegion ChangeRegion_Complement new region info = ", $self->Print(0,$self->End()),"\n";
...
#  print "A gbRegion ChangeRegion_Complement new region info = ", $self->Print(0,$self->End()),"\n";
	$self->Region( $self->Print( 0, $self->End() ) );

#  print "B gbRegion ChangeRegion_Complement new region info = ", $self->Print(0,$self->End()),"\n";
	return 1;
}
...
sub ChangeRegion_Diff {
	...
	foreach my $region (@$regions) {
		...
		#      print "old start, end = $region->{start}, $region->{end}\n";
		#      print "start = $q_end - ( $region->{start} - $s_end)\n";
		$region->{start} = $q_end - ( $region->{start} - $s_end );

		#      print "end = $q_end - ( $region->{end} - $s_end)\n";
		$region->{end} = $q_end - ( $region->{end} - $s_end );

		#      print "new start, end = $region->{start}, $region->{end}\n";
	}
...
#
#    if ( defined $self->Complement()){
#        $self->{complement} = undef;
#    }
#    else { $self->Complement("complement");}
#
#    my ($regions, $region, $temp);
#    $regions = $self->{regions};
...
root::print_hashEntries ($items , 3, "thi Item list was created from the string $region" );
	foreach $temp (@$items) {
...
sub getRegionForDrawing {
	...
	#    print "gbRange\n";
	#    my $temp = $self->{regions};
	#    foreach my $fun (@$temp) {
	#       print "start = $fun->{start}; end = $fun->{end}\n";
	#    }
	warn "use of getRegionForDrawing is changed to Region()!\n";
...
sub ParseRegions {
	...
	#    print "Region $regions got parsed to:\n";
	...
	#    print "ParseRegions starting at \$return[$a]\n";
	for ( my $i = 0 ; $i < @regions ; $i++ ) {    # my $temp (@regions){
		@$return[$a] = $self->Itemize_old( $regions[$i] );
		next unless ( defined @$return[$a] );
		push( @starts, @$return[$a]->{start} );
		push( @ends,   @$return[$a]->{end} );
		$a++;
	}

	#    print "s: @starts,e: @ends\n";
	( $start, $end ) = $self->findStartEnd( \@starts, \@ends );
	$self->{regions} = $return;
...
sub Itemize {
	...
	#my (@info, $info, $items, $result, $temp, $last, $final,  @temp);
	my ( $actualItem, @items );
	## that has to be debugged!!
	## the region is a charwise array of the gbRegion string
...
#warn "we have a ' ' in the region - that should not be the case!\n",join("",@$region),"\n";
		}
		if ( @$region[$a] eq '(' ) {    ## neuer Teil!
			push( @items, $actualItem );
			$actualItem = '';
	...
	root::print_hashEntries ( \@items, 3, "the regions after Itemize" );
	#    $items = $self->{items};
	#    if ( $temp eq ")"){
	#        pop @info;
	#        $last = 100;
	#    }
	#    push (@$items, join("",@info));# if ( defined $info[0]);
	#    if ( $last == 100){
	#       @$items[$i++] = ')';
	#    }
	$self->{items} = \@items;
...
sub Itemize_old {
	...
	if ( $region =~ m/[<>]?(\d+)[<>]?\.\.[<>]?(\d+)[<>]?/ ) {
		...
		#             print " point between $1 and $2 ( $region)\n";
		$return = {
			tag       => 'between',
			start     => $1,
			end       => $2,
