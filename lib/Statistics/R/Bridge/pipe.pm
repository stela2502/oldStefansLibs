...
{ package Statistics::R::Bridge::pipe ;
  ...
  sub pipe { 
    ...
    $this->{LOG_DIR} = $args{log_dir} || "$this->{TMP_DIR}/Statistics-R_$$" ;
  ...
  sub DESTROY { 
    ...
    unlink( "$this->{TMP_DIR}/*" );
#    print "I try to delete the outdir $this->{TMP_DIR}/Statistics-R_$$/output\n";
    rmdir( "$this->{TMP_DIR}/Statistics-R_$$/output" );
    rmdir( "$this->{TMP_DIR}/Statistics-R_$$" );
  }
