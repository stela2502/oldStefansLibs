...
my ( $help, $debug, $database, @IDs, $path, $log_transform );
...
unless ( defined $path ) {
	print helpString(
		"we need to know where to write the pictures to ('outpath')");
	exit;
}
unless ( defined $IDs[1] ) {
	print helpString(
"we need at least two array_calculation_result_IDs in order to compare anything"
	);
	exit;
}
unless ( -d $path ) {
...
sub helpString {
 ...
 command line switches for create_2D_density_plots_4_array_datasets.pl
   ...
   -array_calculation_result_IDs
                   :the IDs from the 'array_datasets' table (min two)
...
my ( $task_description, $loggingTable, $workLoad, $loggingEntries );
...
my ( $array_datasets, $database_interface, $tableDescriptions, $rv, $sth,
	$sql );

## get the search interface
$array_datasets = array_dataset->new( $database, $debug );
( $database_interface, $tableDescriptions ) =
  $array_datasets->getMinimalSearchInterface( \@IDs );
$tableDescriptions = [ $database_interface->Sample_Lables() ];

#die root::get_hashEntries_as_string ({'$database_interface' => $database_interface, '$tableDescriptions' => $tableDescriptions}, 3, "what we got back... ");
## create the SQL statement
$sql = $database_interface->create_SQL_statement(
	{ 'search_columns' => [ 'oligo_name', 'value' ] } );
...
sub createPictures {
	...
	for ( my $i = 0 ; $i < @arrays2compare ; $i++ ) {
		...
		$xyWith_Histo->plot(
			"$path/secondLevelComparison-@$namesArray[0]"
			  . "_@$namesArray[1+$i].svg",
			800, 800, @$namesArray[0], @$namesArray[ 1 + $i ]
		);
	}
	shift(@$namesArray);
	return createPictures( $namesArray, @arrays2compare )
	  if ( @arrays2compare > 1 );
}
