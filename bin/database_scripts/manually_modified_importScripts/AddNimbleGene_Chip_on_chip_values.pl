...
my (
    ...
    $array_dataset_scientist_id,  $scientistTable_name,
    $scientistTable_workgroup,    $scientistTable_position,
    $array_dataset_sample_id,     $sampleTable_sample_lable,
    $sampleTable_subject_id,      $sampleTable_tissue_id,
    $array_dataset_access_right,  $array_dataset_array_id,
    $nucleotide_array_identifier, $array_dataset_experiment_id,
    $experiment_name,             $array_dataset_array_type,
    $data_IP,                     $data_INPUT,
    $data_GFF
);
...
if ( defined $resorce_path ) {
    if ( -f "$resorce_path/data_IP.dta" ) {
        $data_IP = "$resorce_path/data_IP.dta";
    }
    if ( -f "$resorce_path/data_INPUT.dta" ) {
        $data_INPUT = "$resorce_path/data_INPUT.dta";
    }
    if ( -f "$resorce_path/data_GFF.dta" ) {
        $data_GFF = "$resorce_path/data_GFF.dta";
    }
...
my $dataset = {
    ...
    'data' => { 'IP' => $data_IP, 'INPUT' => $data_INPUT, 'GFF' => $data_GFF }
};
...
## work is finfished - we add a log entry and remove the workload entry!
    my $id = $array_dataset->_return_unique_ID_for_dataset($dataset);
...
## work is finfished - we add a log entry and remove the workload entry!
    if ( defined $id && $id > 0 ) {
        ...
        warn
"insert_into_dbTable_tissueTable.pl -> we could not add the data $dataStr\n";
        $errorTable->AddDataset(
            {
                'name'        => 'insert_into_dbTable_tissueTable.pl',
                'description' => $dataStr
            }
        );
    }
...
$errorMessage
 ...
 -data_IP
       the pair file contininge the precipitated sample
 -data_INPUT
       the pair file contininge the input sample
 -data_GFF
       the enrichment factors calculated by NimbleGene
