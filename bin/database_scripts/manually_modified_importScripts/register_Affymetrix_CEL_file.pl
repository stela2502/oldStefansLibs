...
=head1 register_Affymetrix_CEL_file.pl

INFO_STR

To get further help use 'register_Affymetrix_CEL_file.pl -help' at the comman line.
...
my (
	...
	$cel_file, $external_files_file,
	$external_files_filetype,      $sample_id,
	$sampleTable_sample_lable,     $sampleTable_subject_id,
	$sampleTable_tissue_id,        $array_type
);
...
if ( defined $resorce_path ) {
	...
	if ( -f "$resorce_path/cel_file.dta" ) {
		open( IN, "<$resorce_path/cel_file.dta" );
		foreach (<IN>) {
			chomp($_);
			$cel_file = $_;
			last;
	...
	if ( -f "$resorce_path/sample_id.dta" ) {
		open( IN, "<$resorce_path/sample_id.dta" );
		foreach (<IN>) {
			chomp($_);
			$sample_id = $_;
			last;
	...
	if ( -f "$resorce_path/array_type.dta" ) {
		open( IN, "<$resorce_path/array_type.dta" );
		foreach (<IN>) {
			chomp($_);
			$array_type = $_;
			last;
...
my $CEL_file_storage = CEL_file_storage->new($database_name);
...
my $rv = $workingTable->set_workload(
	{
		'PID'         => $$,
		'programID'   => 'insert_into_dbTable_CEL_file_storage.pl',
...
unless ( defined @$loggingEntries[0] ) {
	...
	$CEL_file_storage->AddDataset($dataset);

	my $id = $CEL_file_storage->_return_unique_ID_for_dataset($dataset);
...
## work is finfished - we add a log entry and remove the workload entry!
	if ( defined $id && $id > 0 ) {
		...
		warn
"insert_into_dbTable_CEL_file_storage.pl -> we could not add the data $dataStr\n";
		$errorTable->AddDataset(
			{
				'name'        => 'insert_into_dbTable_CEL_file_storage.pl',
				'description' => $dataStr
			}
		);
	}
...
$errorMessage
    ...
    command line switches for register_Affymetrix_CEL_file.pl
 ...
 NEEDED values:
 -cel_file
       the CEL file you want to upload
 -array_type
       a string that identifies the Affymetrix array design
 -sample_id
       the link to the sample information
