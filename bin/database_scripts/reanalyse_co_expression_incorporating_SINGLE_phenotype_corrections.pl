...
=head1 reanalyse_co_expression_incorporating_SINGLE_phenotype_corrections.pl
...
In contrast to the script reanalyse_co_expression_incorporating_phenotype_corrections.pl, this script will not craete summaries of phenotypes, but will use the phenotypes separately to calculate the residuale expression changes.

To get further help use 'reanalyse_co_expression_incorporating_SINGLE_phenotype_corrections.pl -help' at the comman line.
...
open( LOG,
	">$outpath/reanalyse_co_expression_incorporating_SINGLE_phenotype_corrections.log"
  )
...
for ( my $i = 0 ; $i < @remove_these_phenotypes ; $i++ ) {
	$temp[0] = $remove_these_phenotypes[$i];
