...
To get further help use 'insert_into_dbTable_phenotype_registration.pl -help' at the comman line.
...
Getopt::Long::GetOptions(
         ...
         "-phenotype_registration_table_name=s"  => \$phenotype_registration_table_name,
...
$phenotype_registration_connection_type = 'local' unless ( defined $phenotype_registration_connection_type);
...
my $value = $phenotype_registration->getDownstreamTable_4_type($dataset->{'name'} );
print "we have created a link to the table ".$value->TableName()."\n";
...
$errorMessage
    ...
    command line switches for insert_into_dbTable_phenotype_registration.pl
 ...
 -phenotype_registration_name
       the name of the dataset (e.g. BMI or length)
 ...
 -phenotype_registration_perl_module_name
       ...
       ( 'ph_age',       'familyHistory',   'binary_mono', 
         'binary_multi', 'continuose_mono', 'continuose_multi')
