...
#my $main_results_table = $interface -> get_data_table_4_search ({
# 	'search_columns' => [],
# 	'where' => [[]],
...
$sql_gbFile_id = $interface -> create_SQL_statement ( {
 	'search_columns' => ['chromosomesTable.id', 'chromosomesTable.chromosome'],
...
my $sql_chromosome_name = $interface -> create_SQL_statement ( {
 	'search_columns' => ['chromosomesTable.chromosome'],
...
$data_table->Add_2_Header('Chromosome');
...
for ( $gbFile_id = 1; $gbFile_id <= $max_gbFile_id; $gbFile_id ++){
	...
	$temp = $sql_chromosome_name;
	$temp =~ s/\?/$gbFile_id/;
	$sth = $interface->{'dbh'}->prepare( $temp );
	unless ( $sth->execute()){
		Carp::confess ( "We have not got a result for query '$sql_gbFile_id;'\n".$interface->{'dbh'}->errstr()."\n");
	}
	($Chromosome) = $sth->fetchrow_array();
