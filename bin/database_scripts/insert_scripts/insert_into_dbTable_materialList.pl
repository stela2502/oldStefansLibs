...
=head1 insert_into_dbTable_materialList.pl

INFO_STR

To get further help use 'insert_into_dbTable_materialList.pl -help' at the comman line.
...
my ( $help, $debug, $database_name, $resorce_path, $materialList_list_id, @materialList_others_id, $materialsTable_name, $materialsTable_OrderNumber, $materialsTable_LotNumber );

Getopt::Long::GetOptions(
         "-materialList_list_id=s"  => \$materialList_list_id,
         "-materialList_others_id=s{,}"  => \@materialList_others_id,
...
if ( -f "$resorce_path/materialList_list_id.dta" ) { 
       open ( IN , "<$resorce_path/materialList_list_id.dta" );
       foreach ( <IN> ){
              chomp($_);
              $materialList_list_id = $_;
              last;
       }
       close ( IN );
 }
if ( -f "$resorce_path/materialList_others_id.dta" ) { 
       open ( IN , "<$resorce_path/materialList_others_id.dta" );
       foreach ( <IN> ){
              chomp($_);
              push (@materialList_others_id, $_);
       }
       close ( IN );
 }
...
my $dataset = { 
'list_id' => $materialList_list_id,
'others_id' => \@materialList_others_id,
};
...
my $materialList = materialList->new ( root::getDBH( 'root', $database_name ), $debug);
...
{
			'PID'       => $$,
			'programID' => 'insert_into_dbTable_materialList.pl',
			'description' =>
			  "the dataset: $dataStr"
		}
);
...
$materialList->AddDataset( $dataset );

my $id =$materialList->_return_unique_ID_for_dataset( $dataset );
...
if ( defined $id && $id > 0 ){
	...
	warn "insert_into_dbTable_materialList.pl -> we could not add the data $dataStr\n";
	$errorTable -> AddDataset ( { 'name' => 'insert_into_dbTable_materialList.pl', 'description' => $dataStr } );
}
...
$errorMessage
    ...
    command line switches for insert_into_dbTable_materialList.pl
 ...
 NEEDED values:
 -materialList_list_id
       
 LINKAGE variables:
 -materialList_others_id
