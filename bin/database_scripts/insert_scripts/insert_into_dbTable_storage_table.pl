...
=head1 insert_into_dbTable_storage_table.pl

INFO_STR

To get further help use 'insert_into_dbTable_storage_table.pl -help' at the comman line.
...
my ( $help, $debug, $database_name, $resorce_path, $storage_table_temperature, $storage_table_building, $storage_table_floor, $storage_table_room, $storage_table_box_label, $storage_table_description );

Getopt::Long::GetOptions(
         "-storage_table_temperature=s"  => \$storage_table_temperature,
         ...
         "-storage_table_box_label=s"  => \$storage_table_box_label,
...
if ( -f "$resorce_path/storage_table_temperature.dta" ) { 
       open ( IN , "<$resorce_path/storage_table_temperature.dta" );
       foreach ( <IN> ){
              chomp($_);
              $storage_table_temperature = $_;
              last;
       }
       close ( IN );
 }
...
if ( -f "$resorce_path/storage_table_box_label.dta" ) { 
       open ( IN , "<$resorce_path/storage_table_box_label.dta" );
       foreach ( <IN> ){
              chomp($_);
              $storage_table_box_label = $_;
              last;
       }
       close ( IN );
 }
...
my $storage_table = storage_table->new ( root::getDBH( 'root', $database_name ), $debug);
...
{
			'PID'       => $$,
			'programID' => 'insert_into_dbTable_storage_table.pl',
			'description' =>
			  "the dataset: $dataStr"
		}
);
...
$storage_table->AddDataset( $dataset );

my $id =$storage_table->_return_unique_ID_for_dataset( $dataset );
...
if ( defined $id && $id > 0 ){
	...
	warn "insert_into_dbTable_storage_table.pl -> we could not add the data $dataStr\n";
	$errorTable -> AddDataset ( { 'name' => 'insert_into_dbTable_storage_table.pl', 'description' => $dataStr } );
}
...
$errorMessage
    ...
    command line switches for insert_into_dbTable_storage_table.pl
 ...
 NEEDED values:
 -storage_table_temperature
       the temperatore of the storage
 ...
 -storage_table_box_label
       the lable of the box in the storage
