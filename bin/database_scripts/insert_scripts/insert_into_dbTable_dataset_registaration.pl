...
=head1 insert_into_dbTable_dataset_registaration.pl

INFO_STR

To get further help use 'insert_into_dbTable_dataset_registaration.pl -help' at the comman line.
...
my ( $help, $debug, $database_name, $resorce_path, $dataset_registaration_description, $dataset_registaration_name, $dataset_registaration_XML_definition, $dataset_registaration_lib_string );

Getopt::Long::GetOptions(
         "-dataset_registaration_description=s"  => \$dataset_registaration_description,
         "-dataset_registaration_name=s"  => \$dataset_registaration_name,
         "-dataset_registaration_XML_definition=s"  => \$dataset_registaration_XML_definition,
         "-dataset_registaration_lib_string=s"  => \$dataset_registaration_lib_string,
...
if ( -f "$resorce_path/dataset_registaration_description.dta" ) { 
       open ( IN , "<$resorce_path/dataset_registaration_description.dta" );
       foreach ( <IN> ){
              chomp($_);
              $dataset_registaration_description = $_;
              last;
       }
       close ( IN );
 }
if ( -f "$resorce_path/dataset_registaration_name.dta" ) { 
       open ( IN , "<$resorce_path/dataset_registaration_name.dta" );
       foreach ( <IN> ){
              chomp($_);
              $dataset_registaration_name = $_;
              last;
       }
       close ( IN );
 }
if ( -f "$resorce_path/dataset_registaration_XML_definition.dta" ) { 
       open ( IN , "<$resorce_path/dataset_registaration_XML_definition.dta" );
       foreach ( <IN> ){
              chomp($_);
              $dataset_registaration_XML_definition = $_;
              last;
       }
       close ( IN );
 }
if ( -f "$resorce_path/dataset_registaration_lib_string.dta" ) { 
       open ( IN , "<$resorce_path/dataset_registaration_lib_string.dta" );
       foreach ( <IN> ){
              chomp($_);
              $dataset_registaration_lib_string = $_;
              last;
       }
       close ( IN );
 }
...
my $dataset = { 
'description' => $dataset_registaration_description,
'name' => $dataset_registaration_name,
'XML_definition' => $dataset_registaration_XML_definition,
'lib_string' => $dataset_registaration_lib_string,
};
...
my $dataset_registaration = dataset_registaration->new (  $database_name );
...
{
			'PID'       => $$,
			'programID' => 'insert_into_dbTable_dataset_registaration.pl',
			'description' =>
			  "the dataset: $dataStr"
		}
);
...
$dataset_registaration->AddDataset( $dataset );

my $id =$dataset_registaration->_return_unique_ID_for_dataset( $dataset );
...
if ( defined $id && $id > 0 ){
	...
	warn "insert_into_dbTable_dataset_registaration.pl -> we could not add the data $dataStr\n";
	$errorTable -> AddDataset ( { 'name' => 'insert_into_dbTable_dataset_registaration.pl', 'description' => $dataStr } );
}
...
$errorMessage
    ...
    command line switches for insert_into_dbTable_dataset_registaration.pl
 ...
 NEEDED values:
 -dataset_registaration_description
       describes the function of that program
 -dataset_registaration_name
       the name of the dataset (has to be unique)
 -dataset_registaration_XML_definition
       the name of the XML file to use in order to build the GUI form and to execute the task
 -dataset_registaration_lib_string
