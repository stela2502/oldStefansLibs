...
=head1 insert_into_dbTable_genomeDB.pl

INFO_STR

To get further help use 'insert_into_dbTable_genomeDB.pl -help' at the comman line.
...
my ( $help, $debug, $database_name, $resorce_path, $genomeDB_version, $genomeDB_organism_id, $organismDB_organism_tag, $genomeDB_creationDate );

Getopt::Long::GetOptions(
         "-genomeDB_version=s"  => \$genomeDB_version,
         "-genomeDB_organism_id=s"  => \$genomeDB_organism_id,
         "-organismDB_organism_tag=s"  => \$organismDB_organism_tag,
         "-genomeDB_creationDate=s"  => \$genomeDB_creationDate,
...
if ( -f "$resorce_path/genomeDB_version.dta" ) { 
       open ( IN , "<$resorce_path/genomeDB_version.dta" );
       foreach ( <IN> ){
              chomp($_);
              $genomeDB_version = $_;
              last;
...
if ( -f "$resorce_path/genomeDB_organism_id.dta" ) { 
       open ( IN , "<$resorce_path/genomeDB_organism_id.dta" );
       foreach ( <IN> ){
              chomp($_);
              $genomeDB_organism_id = $_;
              last;
       }
       close ( IN );
 }
...
if ( -f "$resorce_path/genomeDB_creationDate.dta" ) { 
       open ( IN , "<$resorce_path/genomeDB_creationDate.dta" );
       foreach ( <IN> ){
              chomp($_);
              $genomeDB_creationDate = $_;
              last;
       }
       close ( IN );
 }
...
my $dataset = { 
'version' => $genomeDB_version,
'organism_id' => $genomeDB_organism_id,
'organism' => { 
'id' => $genomeDB_organism_id,
'organism_tag' => $organismDB_organism_tag,
},
'creationDate' => $genomeDB_creationDate,
};
...
my $genomeDB = genomeDB->new (  $database_name );
...
{
			'PID'       => $$,
			'programID' => 'insert_into_dbTable_genomeDB.pl',
			'description' =>
			  "the dataset: $dataStr"
		}
);
...
$genomeDB->AddDataset( $dataset );

my $id =$genomeDB->_return_unique_ID_for_dataset( $dataset );
...
if ( defined $id && $id > 0 ){
	...
	warn "insert_into_dbTable_genomeDB.pl -> we could not add the data $dataStr\n";
	$errorTable -> AddDataset ( { 'name' => 'insert_into_dbTable_genomeDB.pl', 'description' => $dataStr } );
}
...
$errorMessage
    ...
    command line switches for insert_into_dbTable_genomeDB.pl
 ...
 NEEDED values:
 -genomeDB_version
       the version of the genome information
 -genomeDB_creationDate
       the data this version of the genome was released
 LINKAGE variables:
 -genomeDB_organism_id
