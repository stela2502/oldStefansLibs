...
my ( $help, $debug, $database_name, $resorce_path, $array_dataset_scientist_id, $scientistTable_name, $scientistTable_workgroup, $scientistTable_position, $array_dataset_sample_id, $sampleTable_sample_lable, $sampleTable_subject_id, $sampleTable_tissue_id, $array_dataset_access_right, $array_dataset_array_id, $nucleotide_array_identifier, $array_dataset_experiment_id, $experiment_name, $array_dataset_array_type );
...
if ( defined $id && $id > 0 ){
	...
	warn "insert_into_dbTable_array_dataset.pl -> we could not add the data $dataStr\n";
	$errorTable -> AddDataset ( { 'name' => 'insert_into_dbTable_array_dataset.pl', 'description' => $dataStr } );
}
