...
$task_description .= 'fetch_raw_data_MeDIP_test.pl';
...
open (LOG ,">$outpath/fetch_raw_data_MeDIP_test.log" ) or die "I could not create the log file $outpath/fetch_raw_data_MeDIP_test.log\n$!\n";
...
sub calculate_Wilcox {
	...
	foreach ( @{ $phenotypes->{$group_1} } ){
		$data_table->{'accepted_new'} ->{$_} = 1;
	}
	foreach ( @{ $phenotypes->{$group_2} } ){
		$data_table->{'accepted_new'} ->{$_} = 1;
	}
	
	
	$line_count = 0;
	my ( $log_val, $groupA, $groupB, $n ,$stdA, $stdB, $data_hash );
	foreach my $oligoName (@$OligoIDs) {
		...
		for ( my $i = 0 ; $i < @{$phenotypes->{$group_1}}; $i ++ ){
			$data_hash->{@{$phenotypes->{$group_1}}[$i]} = @$groupA[$i];
		}
		for ( my $i = 0 ; $i < @{$phenotypes->{$group_2}}; $i ++ ){
			$data_hash->{@{$phenotypes->{$group_2}}[$i]} = @$groupB[$i];
		}
		
		($groupA, $n ,$stdA)  = root->getStandardDeviation($groupA);
		($groupB, $n ,$stdB) = root->getStandardDeviation($groupB);
		
		$data_hash->{'mean1'} = $groupA;
		$data_hash->{'mean2'} = $groupB;
		$data_hash->{'std1'} = $stdA;
		$data_hash->{'std2'} = $stdB;
		$data_hash->{'Oligo_id'} = $oligoName,
		$data_hash->{'p value'} = -1;
		$groupB = 0.000001 if ( $groupB == 0);
		$data_hash->{'fold_change [A/B]'} = ( $groupA / $groupB );
		...
		if ( $line_count % 5000 == 0 ) {
			print
"we have done $line_count calculations - still alive! (oligo_name = $oligoName)\n";
			if ($debug) {
				print
