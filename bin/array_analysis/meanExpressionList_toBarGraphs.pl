...
=head1 meanExpressionList_toBarGraphs.pl

This script plots for each gene in the mean expression list a bargraph representation of the 'expression level'. 


To get further help use 'meanExpressionList_toBarGraphs.pl -help' at the comman line.
...
my (
	...
	@secondary_names, @data_entries,   $dataset, $std_columnNr, @std_value, @order_on_the_plot
);
...
while (<IN>) {
	...
	if ( $lineCount == $headingLine ) {
		...
		$std_columnNr = $tableHandling->identify_columns_of_interest_patternMatch( $_,
			$std_name ) if ( defined $std_name );
	...
	$secondary_names[0] = "dataset" unless ( defined $secondary_names[0]);
	@data_entries =
	  $tableHandling->get_column_entries_4_columns( $_, $dataColumns ); ## here we can have only 1!!
	unless ( defined $dataHash->{ $primary_names[0] } ) {
		$dataHash->{ $primary_names[0] } = {};
	}
	@std_value = $tableHandling->get_column_entries_4_columns( $_, $std_columnNr ) if ( ref($std_columnNr) eq "ARRAY");
	...
	if ( defined $dataHash->{ $primary_names[0] }->{ $secondary_names[0] } ) {
		...
		if ( defined $std_value[0]){
		$dataHash->{ $primary_names[0] }->{ $secondary_names[0] } = { 'y' => $data_entries[0], 'std' => $std_value[0] };
		}
		else {
			$dataHash->{ $primary_names[0] }->{ $secondary_names[0] } = { 'y' => $data_entries[0]};
		}
		push ( @order_on_the_plot, $secondary_names[0]);
	}
}
...
foreach my $firstKey ( sort keys %$dataHash ) {
	...
	$simpleBarGraph->AddDataset(
			{
				'name'  => $firstKey,
				'data'  => $dataHash->{$firstKey},
				'color' => $color->getNextColor(),
				'order_array' => \@order_on_the_plot
			}
		);
...
sub helpString {
	...
	return "
 $errorMessage
 command line switches for meanExpressionList_toBarGraphs.pl
   ...
   -std_col_name        :a optional column name containing std dev values
   ...
   -y_res               :the y resolution in pixel  -help                :print this help
