...
$kegg_genes = kegg_genes->new(root::getDBH('',$debug),$debug);
...
$workingTable->set_workload(
	{
		'PID'         => $$,
		'programID'   => 'import_KEGG_pathway.pl',
...
unless ( defined @$loggingEntries[0] ) {
	...
	PROCESS_PATHWAYS: foreach $PathWay_ID ( keys %$data ){
		...
		$dataset->{'pathway'}->{'picture'}->{'file'} = $data->{$PathWay_ID}->{'image'};
			...
			if ( $_ =~ m/shape=rect.*coords=(\d+),(\d+),(\d+),(\d+).*href=.*title="(.+)"/){
				...
				foreach my $gene_rep ( split(", ",$5)){
					...
					unless ($gene_rep =~ m/(\w+\d+) \((.*)\)/){
						Carp::confess( "we can not process the gene names '$gene_rep'\n" );
					}
...
print "we inserted $i pathways and a total of $genes genes into the KEGG database\n";
